...
export class DrizzleTeacherRepository implements TeacherRepository {
  ...
  async update(teacher: Teacher): Promise<void> {
    await this.drizzleService.db
      ...
      .set({ name: teacher.name, email: teacher.email, document: teacher.document, updatedAt: new Date() })
      .where(eq(teachersSchema.id, teacher.id!));
  }

  async delete(id: string): Promise<void> {
    await this.drizzleService.db.delete(teachersSchema).where(eq(teachersSchema.id, id));
  }

  async findById(id: string): Promise<Teacher | null> {
    const result = await this.drizzleService.db
      .select().from(teachersSchema).where(eq(teachersSchema.id, id)).limit(1);
    return Teacher.restore(result[0]);
  }

  async findByEmail(email: string): Promise<Teacher | null> {
    const result = await this.drizzleService.db
      .select().from(teachersSchema).where(eq(teachersSchema.email, email.toLowerCase())).limit(1);
    return Teacher.restore(result[0]);
  }
